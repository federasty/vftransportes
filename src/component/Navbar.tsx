...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Activa el fondo sólido después de 20px de scroll
  ...
  // 2. Manejo de WhatsApp (Ajustado para un mensaje más específico) 📞
  const handleWhatsAppClick = useCallback(() => {
    const numero = '59894044545'; 
    // Mensaje solicitado: el usuario completa el tipo de transporte
  ...
  // Componente de botón de navegación reutilizable (Estilo Premium/Glow)
  const NavButton: React.FC<{ item: NavItem }> = ({ item }) => (
    <button
      key={item.id}
      // NOTA: En una aplicación real de React, esto debería ser un componente Link/a y usar hash o rutas para navegación
  ...
  return (
    <>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled
        ...
        {/* Elegant Top Bar (Información de Contacto y Estado) */}
        <div className="border-b border-white/5 bg-gradient-to-r from-black via-gray-900/50 to-black">
          <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
            <div className="flex items-center justify-between h-8 sm:h-10 text-[10px] sm:text-xs text-white">
              ...
              {/* Información de Contacto (Visible en Móvil) */}
              ...
              {/* Servicio Disponible (Efecto Pulsante) */}
        ...
        {/* Main Premium Navbar (Contenido de Logo y Navegación) */}
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          <div className="flex items-center justify-between h-20 sm:h-24 lg:h-28">

            {/* Ultra Premium Logo con Efecto Hover y Glow */}
            <div className="flex items-center gap-3 sm:gap-4 lg:gap-6 group cursor-pointer">
              <div className="relative">
                ...
                <div className="absolute inset-0 rounded-full">
                  {/* Animación de rotación solo al hacer hover */}
                ...
                {/* Main Logo - Contenedor con sombras y borde */}
                ...
                {/* Animated Dots (Indicador de actividad visual) */}
              ...
              {/* Título de la empresa (Efecto Gradiente) */}
            ...
            {/* Premium Mobile Menu Button (Toggle) */}
            <button
        ...
        {/* Elite Mobile Menu (Transición Suave) */}
        <div className={`xl:hidden transition-all duration-700 ease-in-out overflow-hidden ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'
          ...
          <div className="px-3 sm:px-4 pt-4 sm:pt-6 pb-6 sm:pb-8 space-y-1.5 sm:space-y-2 bg-black/98 backdrop-blur-2xl border-t border-white/10">
            {navItems.map((item, index) => (
              ...
              >
                {/* Indicador de barra lateral activa (Vertical) */}
      ...
      >
        ...
        {/* Contenido principal del botón (Icono y fondo) */}
        ...
        {/* Texto de Cotización al pasar el mouse (Tooltip) */}
